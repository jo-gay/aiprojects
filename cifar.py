...
def myGetModel(data=CIFAR(7)):
    import keras
    model = keras.Sequential([ 
        keras.layers.Conv2D(64, (3, 3), input_shape=(3, 32, 32), activation='relu'),
...
def myFitModel(model=None, data=CIFAR(7)):
    import keras
    model.fit(data.x_train, data.y_train, epochs=100, batch_size=250, shuffle=True, 
      callbacks=[keras.callbacks.EarlyStopping(min_delta=0.001, patience=20),
                 keras.callbacks.ModelCheckpoint('weights-best.hdf5', save_best_only=True, save_weights_only=True)],
